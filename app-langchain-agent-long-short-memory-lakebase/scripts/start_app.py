...
class ProcessManager:
    ...
    def run(self, backend_args=None):
        ...
        try:
            ...
            # Strip database-related env vars so the frontend (Drizzle ORM) doesn't
            # try to run migrations against the Lakebase instance used by the backend.
            db_keywords = ("DATABASE", "LAKEBASE", "POSTGRES", "PGHOST", "PGPORT",
                           "PGUSER", "PGPASSWORD", "PGDATABASE", "PG_")
            frontend_env = {
                k: v for k, v in os.environ.items()
                if not any(kw in k.upper() for kw in db_keywords)
            }
